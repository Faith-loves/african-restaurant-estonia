...
export default function AdminUsers() {
  ...
  async function invite(event: React.FormEvent) {
    ...
    setInviting(true);
    try { const data = await call("/api/admin/admins", { method: "POST", body: JSON.stringify({ name, email, permissions }) }); setResetLink(data.passwordResetLink); setName(""); setEmail(""); setMessage("Administrator created and invitation email sent. A one-time reset link is also shown below."); await load(); } catch (error) { const apiError = error as Error & { data?: { passwordResetLink?: string } }; if (apiError.data?.passwordResetLink) setResetLink(apiError.data.passwordResetLink); setMessage(apiError.message); } finally { setInviting(false); }
  }
